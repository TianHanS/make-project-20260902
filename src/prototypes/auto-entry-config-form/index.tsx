...
import { Copy, GitBranch, Search, Settings, Settings2, SlidersHorizontal } from 'lucide-react';
import {
    INITIAL_MODULES,
...
export default function Component() {
    ...
    const openCopy = (m: ModuleItem) => {
        if (!hasSavedConfig(configs[m.id])) {
    ...
    const flowInitial = flowModule
        ? configs[flowModule.id]
            ? {
                  sceneSource: configs[flowModule.id].sceneSource,
                  values: configs[flowModule.id].values,
              }
            : null
        : null;
            ...
            <header className="ae-app-head">
                <div className="ae-brand">
                    ...
                    <div>
                        ...
                        <p className="ae-brand-sub">AER 自动入厂 · 场景表单配置</p>
                    </div>
                </div>
            </header>
                    ...
                    <div className="ae-card">
                        ...
                        <div className="ae-module-grid">
                            {pageItems.map((m) => {
                                ...
                                const configured = hasSavedConfig(configs[m.id]);
                                return (
                                    <article key={m.id} className="ae-module-card">
                                        ...
                                        <footer className="ae-module-card-foot aef-card-foot">
                                            {isAer ? (
                                                <>
                                                    <Button
                                                        size="small"
                                                        icon={<SlidersHorizontal size={13} />}
                                                        onClick={() => setFlowModule(m)}
                                                    >
                                                        流程配置
                                                    </Button>
                                                    <Button
            ...
            <CopyFlowDrawer
                ...
                onConfirmCopy={(targetId) => {
                    ...
                    const sourceCfg = configs[copyModule.id] ?? {
                        moduleId: copyModule.id,
                        sceneSource: 'custom' as const,
                        values: createDefaultFormValues(),
                    };
                    persist({
                        ...configs,
                        [targetId]: cloneConfig(sourceCfg, targetId),
                    });
